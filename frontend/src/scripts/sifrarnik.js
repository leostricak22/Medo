  ...
  if (svi_sifrarnici.length > 0) {
    for (let i = 0; i < svi_sifrarnici.length; i++) {
      if (svi_sifrarnici[i].id.includes("jqxgrid" + sifrarnik_id)) {
        let naziv_tablice = svi_sifrarnici[i].onclick
          .toString()
          .split("sifrarnik_prozor_dodaj('")[1]
          .split("'")[0];

        let label_id = $("#" + svi_sifrarnici[i].id)
          .parent()
          .find("label")[0].id;
        let input_id = $("#" + svi_sifrarnici[i].id)
          .parent()
          .find("input")[0].id;
        let polje_tekst = $(
          "#" + input_id.replace("ParametarLxV", "TekstLxV")
        ).text();

        let polje_za_dodati = label_id
          .split(sifrarnik_id + "UrediLxV")[1]
          .split("LabelaLxV")[1]
          .split("ParametarLxV")[0];
        let odabrani_redak = DataAdapter[naziv_tablice].records.find(function (
          item,
          i
        ) {
          return (
            item[polje_za_dodati].toString() ===
            $("#" + input_id)
              .val()
              .toString()
          );
        ...
        if (odabrani_redak === undefined) {
          ...
          $(".errorlabel").text(
            "Odabrana je kriva šifra kod polja: " + polje_tekst + "!"
          );
          return 0;
        }
      }
...
async function prikazi_podatak_label(element, sifrarnik_id) {
  ...
  try {
    label_id = $("#" + element)
      ...
      .parent()
      .find("label")[0].id;
    naziv_tablice = $("#" + element)
      .parent()
      .parent()
      .find(".LimexSifrarnik")[0]
      .onclick.toString()
      .split("sifrarnik_prozor_dodaj('")[1]
      .split("'")[0];
  } catch {
  ...
  let polje_za_dodati = label_id
    ...
    .split("LabelaLxV")[1]
    .split("ParametarLxV")[0];
  let polje_za_prikazati = label_id
    .split(sifrarnik_id + "UrediLxV")[1]
    .split("LabelaLxV")[1]
    .split("ParametarLxV")[1];
  ...
  try {
    ...
    console.log(
      sifrarnici_odabir[naziv_tablice][polje_za_urediti],
      sifrarnici_odabir[$("#" + sifrarnik_id).attr("class")][polje_za_dodati]
    );
    console.log(!sifrarnici_mod_uredivanja_novo[sifrarnik_id]);
    if (
      sifrarnici_odabir[naziv_tablice][polje_za_urediti] !==
        sifrarnici_odabir[$("#" + sifrarnik_id).attr("class")][
          polje_za_dodati
        ] ||
      sifrarnici_mod_uredivanja_novo[sifrarnik_id] === true
    ) {
      try {
        window["data_forward_to_other_input_" + parent_sifrarnik_naziv](
          sifrarnik_id,
          naziv_tablice
        );
      } catch {}
...
async function load_sifrarnik(sifrarnik_id, sifrarnik_naziv) {
  ...
  let novasirina = $("#" + sifrarnik_id)
    .parent()
    .outerWidth();
  let novavisina =
    $("#" + sifrarnik_id)
      .parent()
      .outerHeight() * 0.97; // navbar zauzima 3vh (3%)
  ...
  try {
    polja_za_maknuti = window[sifrarnik_naziv + "_MakniPoljaGrid"](
      sifrarnici_polja[sifrarnik_id]
    );
  } catch {}
  ...
  for (let i = 0; i < sifrarnici_polja[sifrarnik_id].length; i++) {
    ...
    let polje_text = document.getElementById(
      "jqxgrid" +
        sifrarnik_id +
        "UrediLxVTekstLxV" +
        sifrarnici_polja[sifrarnik_id][i]
    ).innerHTML;
  ...
  async function prozorUredivanjeOtvori(rowindex) {
    ...
    let dataRecord = $("#jqxgrid" + sifrarnik_id).jqxGrid(
      "getrowdata",
      editrow
    );
    oldrow = dataRecord;
    for (let i = 0; i < sifrarnici_polja[sifrarnik_id].length; i++) {
      ...
      $(
        "#jqxgrid" +
          ...
          "UrediLxVParametarLxV" +
          sifrarnici_polja[sifrarnik_id][i]
      ).val(dataRecord[sifrarnici_polja[sifrarnik_id][i]]);

      if (
        (input_type == "date" || input_type == "datetime") &&
        dataRecord[sifrarnici_polja[sifrarnik_id][i]] !== ""
      ) {
        $(
          "#jqxgrid" +
            sifrarnik_id +
          ...
          "setDate",
          LimexDatumParseHRV(dataRecord[sifrarnici_polja[sifrarnik_id][i]])
        );
      }
    }
  ...
  async function prozorDodajOtvori() {
    ...
    try {
      window["novo_postavi_pocetnu_vrijednost" + sifrarnik_naziv](
        "#jqxgrid" + sifrarnik_id + "UrediLxVParametarLxV"
      );
    } catch {}
  ...
  $("#jqxgrid" + sifrarnik_id + "popupUredivanje").on("close", function () {
    setTimeout(function () {
      $("#jqxgrid" + sifrarnik_id + "popupUredivanjeForma").jqxValidator(
        "hide"
      );
    }, 300);
    ...
    $("#jqxgrid" + sifrarnik_id).jqxGrid(
      "selectrow",
      sifrarnici_odabir[sifrarnik_naziv].boundindex
    );
    $("#jqxgrid" + sifrarnik_id).jqxGrid(
      "ensurerowvisible",
      sifrarnici_odabir[sifrarnik_naziv].boundindex
  ...
  $("#jqxgrid" + sifrarnik_id + "UredivanjeSave").click(function () {
    if (
      ...
      provjeri_podatke_sifrarnika(sifrarnik_id)
    ) {
      // UREDIVANJE / NOVO - UPISANI PODACI SA POPUP PROZORA
      let newrow = {};
      for (let i = 0; i < sifrarnici_polja[sifrarnik_id].length; i++)
        newrow[sifrarnici_polja[sifrarnik_id][i]] = $(
          "#jqxgrid" +
            sifrarnik_id +
            "UrediLxVParametarLxV" +
            sifrarnici_polja[sifrarnik_id][i]
      ...
      if (sifrarnici_mod_uredivanja_novo[sifrarnik_id]) {
        ...
        let data_to_send = {};
        for (let i = 0; i < sifrarnici_polja[sifrarnik_id].length; i++)
          data_to_send[sifrarnici_polja[sifrarnik_id][i]] =
            newrow[sifrarnici_polja[sifrarnik_id][i]];

        let parametar_dodatak = "";

        try {
          data_to_send = Object.assign(
            data_to_send,
            window[sifrarnik_naziv + "_AddDataToSend"]()
          );
        } catch {}

        try {
          parametar_dodatak =
            window[sifrarnik_naziv + "_Posalji_Parametar_URL"]();
        } catch {}
        ...
        $.ajax({
          url:
            backend_url +
            "/" +
            sifrarnik_naziv.toLowerCase() +
            "/insert" +
            parametar_dodatak,
          type: "POST",
          data: data_to_send,
        }).done(function (data) {
          if (data === "0") alert("Došlo je do greške!");
          else {
        ...
        data_to_send[sifrarnici_polja[sifrarnik_id][0] + "Old"] =
          oldrow[sifrarnici_polja[sifrarnik_id][0]];
  ...
  // DESNI KLIK - START
  let contextMenu = $("#jqxgrid" + sifrarnik_id + "popupMenu").jqxMenu({
    width: 200,
    autoOpenPopup: false,
    mode: "popup",
  });
  ...
  $("#jqxgrid" + sifrarnik_id + "popupMenuUredi").click(async function () {
    let selectedrowindex = $("#jqxgrid" + sifrarnik_id).jqxGrid(
      "getselectedrowindex"
    );
  ...
  $("#jqxgrid" + sifrarnik_id + "gumbDodaj").jqxButton({
    width: sirina_gumba,
    height: 20,
  });
  $("#jqxgrid" + sifrarnik_id + "gumbUredi").jqxButton({
    width: sirina_gumba,
    height: 20,
  });
  $("#jqxgrid" + sifrarnik_id + "gumbObrisi").jqxButton({
    width: sirina_gumba,
    height: 20,
  });
  $("#jqxgrid" + sifrarnik_id + "gumbIspisi").jqxButton({
    width: sirina_gumba,
    height: 20,
  });
  $("#jqxgrid" + sifrarnik_id + "gumbOdaberi").jqxButton({
    width: sirina_gumba,
    height: 20,
  });

  if (sifrarnik_id !== "Donji") {
    $("#jqxgrid" + sifrarnik_id + "gumbDodaj")
      .find("span")
      .append(" (f1)");
    $("#jqxgrid" + sifrarnik_id + "gumbUredi")
      .find("span")
      .append(" (f2)");
    $("#jqxgrid" + sifrarnik_id + "gumbIspisi")
      .find("span")
      .append(" (f7)");
  } else {
    $("#jqxgrid" + sifrarnik_id + "gumbDodaj")
  ...
  if (
    ["Gornji", "Donji", "slave", "master"].includes(sifrarnik_id) ||
    sifrarnici_id[0] == sifrarnik_id
  )
  ...
  $("#jqxgrid" + sifrarnik_id + "gumbOdaberi").click(function () {
    ...
    let window_container_id =
      document.getElementById(sifrarnik_id).parentNode.parentNode.parentNode.id;
    $("#" + window_container_id).jqxWindow("close");

    let parent_sifrarnik = window_container_id.substring(
      6,
      window_container_id.indexOf("SifrarnikLxV")
    );
    ...
    $(
      "#jqxgrid" +
        parent_sifrarnik +
        "UrediLxVParametarLxV" +
        sifrarnici_odabir[sifrarnik_naziv].polje_za_urediti
    ).val(
      sifrarnici_odabir[sifrarnik_naziv][
        sifrarnici_odabir[sifrarnik_naziv].polje_za_dodati
      ]
    );
    ...
    for (let i = 0; i < sve_labele.length; i++) {
      $("#" + sve_labele[i].id).text(
        sifrarnici_odabir[sifrarnik_naziv][
          sve_labele[i].id.split(
            "jqxgrid" +
              parent_sifrarnik +
              "UrediLxV" +
              sifrarnici_odabir[sifrarnik_naziv].polje_za_urediti +
  ...
  try {
    $(
      "#" +
        document.getElementById(sifrarnik_id).parentNode.parentNode.parentNode
          .id
      ...
      $("#jqxgrid" + sifrarnici_id[sifrarnici_id.length - 1]).jqxGrid(
        "updatebounddata"
      );
    });
  } catch {}
  ...
  $("#jqxgrid" + sifrarnik_id + "PitanjeBrisanje").on(
    "close",
    function (event) {
      if (event.args.dialogResult.OK) {
        let rowindex = $("#jqxgrid" + sifrarnik_id).jqxGrid(
          "getselectedrowindex"
        );
        let id = $("#jqxgrid" + sifrarnik_id).jqxGrid("getrowid", rowindex);

        let rowSelektirani = $("#jqxgrid" + sifrarnik_id).jqxGrid(
          "getrowdata",
          rowindex
        );

        let data_to_send = {};
        data_to_send[sifrarnici_polja[sifrarnik_id][0]] =
          rowSelektirani[sifrarnici_polja[sifrarnik_id][0]];
        ...
        $.ajax({
          ...
          if (data === "0") alert("Došlo je do greške!");
            ...
            if (
              $("#jqxgrid" + sifrarnik_id).jqxGrid("getdatainformation")
                .rowscount > rowindex
            ) {
  ...
  $("#jqxgrid" + sifrarnik_id).on("rowselect", async function (event) {
    ...
    $("#jqxgrid" + sifrarnik_id + "Status").html(
      1 +
        parseInt(
          $("#jqxgrid" + sifrarnik_id).jqxGrid(
            "getrowvisibleindex",
        ...
        "/" +
        DataAdapter[sifrarnik_naziv].records.length
    );
  });
  ...
  $(".NextNaEnter").on("keypress", function (e) {
    if (e.which === 13) {
      let vrsta_polja = $(".NextNaEnter").eq(
        $(".NextNaEnter").index(e.delegateTarget) + 1
      );
  ...
  $("body").keydown(function (e) {
    ...
    if (
      sifrarnik_id !== "Donji" &&
      sifrarnici_id.includes(sifrarnik_id) &&
      !otvoreni_sifrarnici.includes(sifrarnik_id)
    ) {
      if (sifrarnik_id === "Gornji") {
        if (otvoreni_sifrarnici.includes("Donji")) return;
        if (e.keyCode === 114) {
          e.preventDefault();
...
async function sifrarnik_dodaj(
  sifrarnik_id,
  parent = "girdContainer",
  naziv = ""
) {
  ...
  if (parent === "body") document.body.appendChild(sifDiv);
  else document.getElementById(parent).appendChild(sifDiv);
  ...
  $("#" + sifrarnik_id)
    .find('[id*="' + trazeni_string + '"]')
    .each(function () {
      let novi_ID = $(this)
        .attr("id")
        .replace(trazeni_string, string_za_promjeniti);
...
async function sifrarnik_prozor_dodaj(naziv_podsifrarnika) {
  ...
  podsifrarnik.innerHTML =
    "<div id='" +
    podsifrarnik_id +
    "Header'>Šifrarnik</div><div style='overflow: hidden' id='" +
    podsifrarnik_id +
    "Content'></div>";
  ...
  let parent_sifrarnik = $(event.target["parentNode"]["parentNode"]).find(
    ".LimexSifrarnik"
  )[0];
  let polje_za_dodati = parent_sifrarnik.id
    .split("UrediLxVSifrarnikLxV")[1]
    .split("Promijeni")[0];
  let polje_za_urediti = parent_sifrarnik.id
    .split("UrediLxVSifrarnikLxV")[1]
    .split("Promijeni")[1];
...
async function jqxGridPozicioniranjeNaKraj(idGrida) {
  ...
  if (visiblerows.length > 0) {
    $("#" + idGrida).jqxGrid(
      "selectrow",
      visiblerows[visiblerows.length - 1].boundindex
    );
    $("#" + idGrida).jqxGrid(
      "ensurerowvisible",
      visiblerows[visiblerows.length - 1].boundindex
...
function jqxGridPozicioniranjePremaVrijednosti(idGrida, podatak, vrijednost) {
  ...
  for (let i = 0; i < rowsCount; i++) {
    ...
    if (
      $("#" + idGrida).jqxGrid("getcelltext", trenutnired, podatak) ===
      vrijednost
    ) {
      $("#" + idGrida).jqxGrid("selectrow", i);
      let selectedrowindex = $("#" + idGrida).jqxGrid("getselectedrowindex");
      let selectedrow = $("#" + idGrida).jqxGrid(
        "getrowvisibleindex",
        selectedrowindex
      );
...
async function jqxGridFokusKontrole(idGrida) {
  try {
    if (idGrida !== "jqxgridDonji") $("#" + idGrida).jqxGrid("focus");
    else $("#jqxgridGornji").jqxGrid("focus");
  } catch {}

  if (
    jqxGridArray[idGrida + "_Vrijednost"] !== "" &&
    jqxGridArray[idGrida + "_Vrijednost"] !== undefined
  ) {
    jqxGridPozicioniranjePremaVrijednosti(
      idGrida,
      jqxGridArray[idGrida + "_Podatak"],
      jqxGridArray[idGrida + "_Vrijednost"]
    );
  } else {
    if (idGrida === "jqxgridDonji")
      await jqxGridPozicioniranjeNaPocetak(idGrida);
    else await jqxGridPozicioniranjeNaKraj(idGrida);
  }
}
